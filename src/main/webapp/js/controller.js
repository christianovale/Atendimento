...
crudAtendimento.controller("AtendimentoController",["$resource", "$scope", function($resource, $scope){
	...
	var angularResource = $resource("/Atendimento/rest/atendimento");
	var angularResourceParam = $resource("/Atendimento/rest/atendimento/:protocolo", {protocolo:"@protocolo"});
	...
	$scope.excluir = function(){
		...
		newResource.$delete(function(){
			$scope.atendimentos.splice($scope.atendimentos.indexOf
					($scope.atendimento),1);
			$scope.novo;
		});
	}
